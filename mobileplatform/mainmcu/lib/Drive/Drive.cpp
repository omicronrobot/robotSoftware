...
Drive::Drive(DrivePins drivePins, StepperPins stepperPins, uint16_t steps_per_rev, float step_delay) : _top_left(drivePins.TopLeft.IN1, drivePins.TopLeft.IN2), _top_right(drivePins.TopRight.IN1, drivePins.TopRight.IN2), _bottom_left(drivePins.BottomLeft.IN1, drivePins.BottomLeft.IN2), _bottom_right(drivePins.BottomRight.IN1, drivePins.BottomRight.IN2), _stepper(steps_per_rev, step_delay, stepperPins.S1, stepperPins.S2, stepperPins.S3, stepperPins.S4)
{
...
void Drive::drive(float pitch, uint16_t yaw)
{
    ...
    this->_change_angle(yaw);
}

void Drive::_change_angle(uint16_t angle)
{
    _stepper.stepperDrive(angle);
}
